...
"""
As said in the note, brute-forcing the solution is too slow and expensive.
Luckily, it is not necessary: The brute-force method computes a lot of stuff
we do not really need. For example: Lets say, the algorithm works on the last
three levels:
  59
 73 41
52 40 9
.. .. ..
and tries to find the maximum path looking at ...-40-73-59. Finishing that, it
later computes the path sums for 40-41-59. As the set of paths coming to 40 are
the same, and the whole path differs just by the element 41 instead of 73,
all of these paths are computed again unnecessarily.
The solution needs to use dynamic programming. This paradigma is used when a
optimal solution can be composed by optimal solutions of partial problems, as here:
If the max path includes the element 40, it can be composed by the optimal path
from 40 and the optimal path to 40 from the first element. Our algorithm uses
this as following: Every element can only be reached by two elements under it.
Thus, we loop over every element, starting at the second level from the bottom,
and determine which element below has the bigger path sum yet. Adding the current
element, we got the max path sum to this element. We loop over every level up to
the root and get the max path sum from the bottom up to the root. We store the
max path sum for every element by changing it from integer to a tuple of two
integers: (element, max_path_sum).
"""
...
def sol():
    ...
    # The max path sum for the first element of the path is the element itself
    for l in range(len(TRI[k])):
        TRI[k][l] = (TRI[k][l], TRI[k][l])

    # We loop from the second level from the bottom to the root
    for i in range(len(TRI)-2, -1, -1):

        # On the level, we loop over every element
        for j in range(len(TRI[i])):

            # We get the max path sums of the elements below the current
            left = TRI[i+1][j][1]
            right = TRI[i+1][j+1][1]

            # And add the biggest one to our element and got the max path sum to our element
            TRI[i][j] = (TRI[i][j], TRI[i][j] + max(left, right))
    
    # The max path sum of the root is our solution
    return TRI[0][0][1]
